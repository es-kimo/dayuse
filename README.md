...
## 🎓 백엔드/DB 핵심 학습 과제 & 셀프 체크

1. **User와 Group 사이의 N:M 관계를 왜 `GroupMember` 연결 엔티티로 매핑했는가?**
2. **`groupId + userId` 복합 유니크 제약조건을 DB 레벨에 왜 걸어야 하는가?**
3. **초대 토큰 만료 및 인가(Authorization) 가드를 백엔드 계층 어디서 검증해야 안전한가?**
